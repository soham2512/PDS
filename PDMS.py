...
loaded_json = open("static/models/crop.json", "r")
...
# load weights
loaded_model.load_weights("static/models/crop_weights.h5")
model1 = load_model("static/models/one-class.h5")

# global graph
graph = tf.get_default_graph()

def info():
    conn = sqlite3.connect("static/models/crop.sqlite")
    cursor = conn.cursor()
...
@app.route('/usePdms', methods=["GET", "POST"])
def loadUsePdms():
    try:
        return render_template("usePdms.html")
        ...
        if request.method == 'POST':
            ...
            if leaf == "leaf":
                ...
                if value:
                    ...
                    input_img_path = "../uploads/" + secure_filename(f.filename)

                    print(input_img_path)
                return render_template('result.html', Pathogen=Pathogen, Symptoms=Symptoms, Management=Management,
                                       result=Disease, input_img_path=input_img_path)
            else:
                return render_template('usePdms.html',
                                       Error="ERROR: UPLOADED IMAGE IS NOT A LEAF (OR) MORE LEAVES IN ONE IMAGE")
                # return result
        return None
    except Exception as Ex:
        print(Ex)
